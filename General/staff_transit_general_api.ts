...
import {SettingRoute, VerificationRoute} from "./routes";
import cors from "cors";
import session from "express-session";
import {publishToTopic} from "./events/kafka";
...
const port = process.env.PORT || 90; // config.get("port") as number;
...
app.listen(port, () => {
  ...
  VerificationRoute(app);
  SettingRoute(app);
   publishToTopic('topic-2', 'Hello, world!');
});
